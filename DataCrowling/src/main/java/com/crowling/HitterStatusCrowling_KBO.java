...
public class HitterStatusCrowling_KBO {
	...
	public HitterStatusCrowling_KBO() {
		//WebDriver 경로 설정
		...
		// 크롤링 할 주소
		hitterUrl = "https://www.koreabaseball.com/Record/Player/HitterBasic/Basic1.aspx";
	}
	...
	public void activateBot() {
		try {
	        ...
	        try (Workbook workbook = new XSSFWorkbook()) {
	            // Excel 시트 생성
	            Sheet sheet = workbook.createSheet("24전체타자");
				...
				/*
				 * // 첫 번째 행에 열의 이름 추가 Row headerRow = sheet.createRow(rowNum++); String[]
				 * headers = {"rank", "p_no", "name", "year", "position", "team", "WAR", "G",
				 * "PA", "ePA", "AB", "R", "H", "2B", "3B", "HR", "TB", "RBI", "SB", "CS", "BB",
				 * "HP", "IB", "SO", "GDP", "SH", "SF", "AVG", "OBP", "SLG", "OPS", "R/ePA",
				 * "wRC+", "WAR"}; for (int i = 0; i < headers.length; i++) { Cell cell =
				 * headerRow.createCell(i); cell.setCellValue(headers[i]); }
				 */
	            ...
	            // Excel 파일로 저장
	            try (FileOutputStream outputStream = new FileOutputStream("2024_KBO_Hitter_Status.xlsx")) {
	...
	public static void main(String[] args) {
		...
		HitterStatusCrowling_KBO psc = new HitterStatusCrowling_KBO();
		psc.activateBot();
